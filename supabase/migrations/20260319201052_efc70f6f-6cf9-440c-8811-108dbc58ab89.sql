...
COMMENT ON COLUMN public.accounts.include_in_summary IS
  'Si false, la cuenta no suma al total de activos/pasivos del Dashboard. Sigue visible en el detalle.';

-- Update the RPC to include the new field in accounts_summary
CREATE OR REPLACE FUNCTION public.get_dashboard_summary(p_start_date date DEFAULT NULL::date, p_end_date date DEFAULT NULL::date)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
  ...
  SELECT jsonb_build_object(
    ...
    'upcoming_debts', (
      ...
      FROM (
        ...
        FROM public.debts
        WHERE user_id = v_user_id AND is_active = true AND due_day IS NOT NULL
        ORDER BY due_day
      ) t
    ),
    'upcoming_goals', (
      ...
      FROM (
        ...
        WHERE user_id = v_user_id AND is_active = true AND contribution_day IS NOT NULL
