...
export function TopCategoriesCard({ categories }: TopCategoriesCardProps) {
  if (categories.length === 0) {
    return (
      <div className="rounded-xl bg-card border border-border p-4">
        <h3 className="text-sm font-heading font-semibold text-foreground mb-2">Top categorías</h3>
  ...
  return (
    <div className="rounded-xl bg-card border border-border p-4 space-y-3">
      <h3 className="text-sm font-heading font-semibold text-foreground">Top categorías del mes</h3>
      <div className="space-y-2">
        {categories.map((cat, i) => (
          <div key={cat.name} className="flex items-center gap-2">
            <span className="text-[10px] text-muted-foreground w-4 text-right shrink-0">{i + 1}</span>
            <span className={cn("h-2 w-2 rounded-full shrink-0", blockColors[cat.block] || "bg-muted")} />
